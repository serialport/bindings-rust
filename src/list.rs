use napi::{self, CallContext, Env, JsObject, Result, Status};
use serialport::{available_ports, SerialPortType};
use tokio::{self, task};

async fn async_list() -> Result<Vec<serialport::SerialPortInfo>> {
    let handel = task::spawn_blocking(|| available_ports());
...
fn list_to_js_objects(env: &mut Env, data: Vec<serialport::SerialPortInfo>) -> Result<JsObject> {
    ...
    for (i, port_info) in data.iter().enumerate() {
        ...
        js_port_info.set_named_property(
            "portType",
            env.create_string(&format!("{:?}", port_info.port_type))?,
        )?;
        ...
        if let SerialPortType::UsbPort(usb_info) = port_info.port_type.clone() {
            ...
            if let Some(manufacturer) = usb_info.manufacturer {
                js_port_info
                    .set_named_property("manufacturer", env.create_string(&manufacturer)?)?
            }

            if let Some(serial_number) = usb_info.serial_number {
                js_port_info
                    .set_named_property("serialNumber", env.create_string(&serial_number)?)?
            }
...
pub fn list(ctx: CallContext) -> Result<JsObject> {
    ctx.env
        .execute_tokio_future(async_list(), list_to_js_objects)
}
